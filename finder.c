...
int main(int argc, char *argv[])
{
  ...
  int p1[2], p2[2], p3[2]; // casually commenting our the fourth pipe p4[2];
  ...
  if (pid_1 == 0) {
    ...
    //STEP 2
		...
		//And, close all other pipe ends except the ones used to r  direct the above OUTPUT (very important)
    char cmdbuf[ BSIZE ];
    bzero( cmdbuf, BSIZE );
    sprintf( cmdbuf, "%s %s -name \'*\'.[ch]", FIND_EXEC, argv[ 1 ] );

    dup2( p1[ 1 ], STDOUT_FILENO ); // out to p1 write
  ...
  if (pid_2 == 0) {
    ...
    dup2( p1[ 0 ], STDIN_FILENO ); // in from p1 read
    dup2( p2[ 1 ], STDOUT_FILENO ); // out to p2 write
  ...
  if (pid_3 == 0) {
    ...
    dup2( p2[ 0 ], STDIN_FILENO ); // in from p2 read
    dup2( p3[ 1 ], STDOUT_FILENO ); // out to p3 write
  ...
  close( p3[ 1 ] ); // close write end of pipe 3
  ...
  if (pid_4 == 0) {
    ...
    dup2( p3[ 0 ], STDIN_FILENO ); // in from p3 read
  ...
  close( p3[ 0 ] ); // close read end of pipe 3
